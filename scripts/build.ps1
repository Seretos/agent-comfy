# Build the agent-comfy MCP server into a single-file binary.
#
...
# extensionless `bin/comfy`; each per-OS build job emits its own
...
#   pwsh -File scripts/build.ps1 -Package    # also stage build/stage/agent-comfy/
#
...
Invoke-Py -m PyInstaller comfy.spec --clean --noconfirm
if ($LASTEXITCODE -ne 0) {
...
if ($IsWindows) {
    ...
    # If the lock turns out to be a running comfy.exe (i.e. the dev's
    ...
    if (-not $copied) {
        $running = @(Get-Process -Name comfy -ErrorAction SilentlyContinue)
        if ($running.Count -gt 0) {
            $procPids = ($running | ForEach-Object { $_.Id }) -join ", "
            Write-Host "    comfy.exe is still running (PID: $procPids)." -ForegroundColor Yellow
            Write-Host "    A Claude Code session likely has the plugin's MCP server loaded."
            Write-Host "    Close it (or run '/mcp' and disconnect 'comfy') and re-run the build."
            Write-Host "    To kill it now without that:   Stop-Process -Name comfy -Force"
        }
...
# 7. Optional: stage build/stage/agent-comfy/ for the assembly step in
...
if ($Package) {
    ...
    $stage = Join-Path $root "build/stage/agent-comfy"
    ...
    Write-Host "    build/stage/agent-comfy (this-OS payload only)"
}

Write-Step "Done."
Write-Host "bin/$ExeName is ready. plugin.json points at the extensionless 'bin/comfy' so each OS auto-selects its native binary."
